...
function initializeProductDetailEvents(product, stateManager) {
  ...
  function updateMinusButtonState(currentValue) {
    if (currentValue <= 1) {
      // 1 이하일 때: 비활성화
      newQuantityMinus.disabled = true;
      newQuantityMinus.classList.add("opacity-50", "cursor-not-allowed");
      newQuantityMinus.classList.remove("hover:bg-gray-100");
    } else {
      // 2 이상일 때: 활성화
      newQuantityMinus.disabled = false;
      newQuantityMinus.classList.remove("opacity-50", "cursor-not-allowed");
      newQuantityMinus.classList.add("hover:bg-gray-100");
    }
  }
  ...
  function updatePlusButtonState(currentValue) {
    if (currentValue >= stock) {
      // 재고와 같거나 클 때: 비활성화
      newQuantityPlus.disabled = true;
      newQuantityPlus.classList.add("opacity-50", "cursor-not-allowed");
      newQuantityPlus.classList.remove("hover:bg-gray-100");
    } else {
      // 재고보다 작을 때: 활성화
      newQuantityPlus.disabled = false;
      newQuantityPlus.classList.remove("opacity-50", "cursor-not-allowed");
      newQuantityPlus.classList.add("hover:bg-gray-100");
    }
  }
  ...
  newQuantityMinus.addEventListener("click", () => {
    if (newQuantityMinus.disabled) return;

    const currentValue = parseInt(newQuantityInput.value) || 1;
    if (currentValue > 1) {
      const newValue = currentValue - 1;
  ...
  newQuantityPlus.addEventListener("click", () => {
    if (newQuantityPlus.disabled) return;
